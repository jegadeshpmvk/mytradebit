...
class ChangePasswordFront extends Model
{
    ...
    {
        if (!$this->hasErrors()) {
            $user = Customer::findOne((int) Yii::$app->user->id);
            if ($user) {
    ...
    {
        if ($user === false) {
            $user = Customer::findOne((string) Yii::$app->user->id);
        }
        if ($user) {
    ...
    {
        if (!$this->hasErrors()) {
            $user = Customer::findOne(['email_hash' => $this->token]);
            if ($user) {
                $user->password = User::generatePassword($this->password);
                $user->email_hash = NULL;
                if (!$user->save())
    ...
    {
        if (!$this->hasErrors()) {
            $user = Customer::findOne(['email' => $this->email, 'deleted' => 0]);
            if (!$user)
